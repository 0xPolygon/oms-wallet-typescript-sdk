...
for (const declarationRoot of declarationRoots) {
    if (!fs.existsSync(declarationRoot)) {
        throw new Error(`Missing built declaration entrypoint: ${path.relative(projectRoot, declarationRoot)}`)
    }
...
if (leaks.length > 0) {
    ...
    process.stdout.write(`Public API declaration check passed (${visited.size} declaration files).\n`)
}
...
function checkPublicApiBaseline() {
    ...
    if (!fs.existsSync(publicApiBaseline)) {
        throw new Error('Missing scripts/public-api-baseline.txt. Regenerate with UPDATE_PUBLIC_API_BASELINE=1 pnpm check:public-api.')
    }
    ...
    throw new Error('Packaged public API differs from scripts/public-api-baseline.txt. Review it, then regenerate intentionally with UPDATE_PUBLIC_API_BASELINE=1 pnpm check:public-api.')
}
