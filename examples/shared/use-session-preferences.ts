...
export function useSessionPreferences({
  ...
  useEffect(() => {
    window.sessionStorage.setItem(manualWalletSelectionKey, useManualWalletSelection ? 'true' : 'false')
  ...
  const walletSelection: WalletSelectionBehavior = useManualWalletSelection ? 'manual' : 'automatic'
